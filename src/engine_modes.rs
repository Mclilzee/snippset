...
pub fn start_editing_engine(path: PathBuf) -> Result<(), InquireError> {
...
pub fn add_to_file(path: PathBuf) -> Result<(), InquireError> {
...
pub fn edit_file(path: PathBuf) -> Result<(), InquireError> {
    let file = File::open(&path)?;
    let reader = BufReader::new(file);
